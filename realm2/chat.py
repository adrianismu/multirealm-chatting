...
class RealmCommunicationThread(threading.Thread):
    def __init__(self, chat, target_realm_address, target_realm_port):
        ...
        self.queue = Queue()  # Queue for incoming messages from the other realm
...
class Chat:
	...
	def get_realm_inbox(self, sessionid, realm_id):
		...
		if (realm_id not in self.realms):
			return {'status': 'ERROR', 'message': 'Realm Tidak Ada'}
		messages= self.realms[realm_id].get_messages()
...
if __name__=="__main__":
	...
	# print(j.proses("sendrealm {} realm1 messi hellomes ini dari realm2" . format(tokenid)))
	# print(j.proses("getrealminbox {} realm1" . format(tokenid)))
	# print(j.proses("addrealm realm2 127.0.0.1 8886"))
	# print(j.proses("sendrealm {} realm2 henderson halo son ini dari realm1" . format(tokenid)))
	# print(j.proses("getrealminbox {} realm2" . format(tokenid)))
	# print(j.proses("sendgrouprealm {} realm2 henderson,messi,lineker halo semua" . format(tokenid)))
	# print(j.proses("getrealminbox {} realm2" . format(tokenid)))
	# print(j.proses("sendgrouprealm {} realm1 henderson,messi,lineker halo semua" . format(tokenid)))
	# print(j.proses("getrealminbox {} realm1" . format(tokenid)))